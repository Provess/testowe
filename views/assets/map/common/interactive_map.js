class InteractiveMap {
     ...
     * @param {string} [url=/assets/map/map_tiles/{z}/{x}/{y}.png] Path to tile images
     */
    addTileLayer(name, args, url = `/assets/map/map_tiles/{z}/{x}/{y}.png`) {
        let defaults = {
            minNativeZoom: 3,
            maxNativeZoom: 5,
            noWrap: true,
     ...
     * @param {string | function} [args.sidebar_icon_html=function () { return `<img class="sidebar-image" src="/assets/map/images/icons/${this.id}.png" />`; }] A html string for the sidebar icon. Can be a function which returns a html string. The function has access to values of this layer e.g. the `this.id`.
    ...
    #setUpSidebar(attribution, website, website_subdir) {
        ...
        this.#sidebar.addPanel({
            id: 'ucp',
            tab: '<i class="fas fa-sign-out"></i>',
            position: 'bottom',
            button: () => {
                location.href = "https://ucp.la-rp.pl/"
            }
        });
